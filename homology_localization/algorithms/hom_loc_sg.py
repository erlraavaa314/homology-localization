...
def hom_loc_sg(cc, v_set, give_status, memory_limit):
    ...
    for bag_id in range(0, cc.sg_NTD_size()):  # This ordering of bag ID's works because this ordering is topological.
        ...
        if bag_type == "introduce" and len(children_id) == 1:
            ...
            tables[bag_id], memory = introduce_sg(child_table, bag_id, children_id[0], cc, v, u, memory, memory_limit)
            parsed = True

        # Forgetting an old simplex in the bag
        if bag_type == "forget" and len(children_id) == 1:
            child_table = tables[children_id[0]]
            tables[bag_id], memory = forget_sg(child_table, bag_id, children_id[0], cc, v, u, memory, memory_limit)
            parsed = True
        ...
        if bag_type == "join" and len(children_id) == 2:
            ...
            tables[bag_id], memory = join_sg(child_table1, child_table2, bag_id, child_bag_id, cc, v, u, memory,
                                             memory_limit)
            parsed = True
...
def introduce_sg(child_table, bag_id, child_id, cc, v, u, memory, memory_limit):
    ...
    if sigma_up:
        return introduce_up(child_table, sigma, sigma_set, bag_id, child_id, cc, v, u, memory, memory_limit)
    else:
        return introduce_down(child_table, sigma, sigma_set, bag_id, child_id, cc, v, u, memory, memory_limit)


def introduce_up(child_table, sigma_node, sigma_set_node, bag_id, child_id, cc, v, u, memory, memory_limit):
...
def introduce_down(child_table, rho_node, rho_set_node, bag_id, child_id, cc, v, u, memory, memory_limit):
    ...
    # The d+1-simplices of the child bag
    down_simps = frozenset([simp for simp in simps if not cc.sg_simplex_up(simp)])
    ...
    u[bag_id][1].add_element(rho, cc.get_weight(rho))
    ...
    for q in child_table:
        ...
        for p in child_table[q]:
            if rho_in:
                table[q][u[bag_id][1].union(p, rho_set_int)] = child_table[q][p] + cc.get_weight(rho)
            else:
...
def forget_sg(child_table, bag_id, child_id, cc, v, u, memory, memory_limit):
    ...
    if sigma_up:
        return forget_up(child_table, sigma, sigma_set, bag_id, child_id, cc, v, u, memory, memory_limit)
    else:
        return forget_down(child_table, sigma, sigma_set, bag_id, child_id, cc, v, u, memory, memory_limit)


def forget_up(child_table, sigma_node, sigma_set_node, bag_id, child_id, cc, v, u, memory, memory_limit):
    ...
    for q in child_table:
        ...
        for p in child_table[q]:
            ...
            if p in table[q_sig_int]:
                value_other = table[q_sig_int][p]
                if value < value_other:
                    table[q_sig_int][p] = value
...
def forget_down(child_table, rho_node, rho_set_node, bag_id, child_id, cc, v, u, memory, memory_limit):
    ...
    for q in child_table:
        ...
        for p in child_table[q]:
            ...
            # We check if the new p_sig is already in the list of tuples stored at q_sig. If it is we compare it to the old one and update it if neccessary
            if p_rho_old_int in table[q]:
                value_other = table[q][p_rho_old_int]

                if value < value_other:
                    table[q][p_rho_old_int] = value
...
def join_sg(child_table1, child_table2, bag_id, child_bag_id, cc, v, u, memory, memory_limit):
